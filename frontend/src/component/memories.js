...
function Memo() {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:1337/api/memories?populate=*')
            ...
            <div className="gallery">
                ...
                {memories.map((memory, i) => (
                    <div className={`gallery__item gallery__item--${i}`} key={memory.id}>
                        ...
                        <img className="gallery__img" src={`http://localhost:1337${memory.attributes.memories.data[0].attributes.url}`} alt={memory.attributes.memories.data[0].attributes.name} />
                    </div>
                ))}
            </div>
        </div>
