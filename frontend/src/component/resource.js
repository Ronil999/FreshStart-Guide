...
const Resources = () => {
  const downloadFile = (arg) => {
    ...
    link.href = `http://localhost:1337${arg}`;
    link.download = 'file';
  ...
  const fetchData = () => {
    const link = `http://localhost:1337/api/resources?populate=*&filters[Department][$eq]=${selectedDepartment}&filters[Semester][$eq]=${selectedSemester}&filters[Subject][$eq]=${selectedSubject}`;
